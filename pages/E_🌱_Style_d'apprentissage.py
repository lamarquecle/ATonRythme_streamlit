...
def explorer_repertoire():
    current_path = Path.cwd()

    # Fichiers et dossiers dans le répertoire actuel
    fichiers_actuels = [p.name for p in current_path.iterdir() if p.is_file()]
    dossiers_actuels = [p.name for p in current_path.iterdir() if p.is_dir()]

    # Répertoires parents
    parents = list(current_path.parents)

    # Enfants des parents (contenu des répertoires parents)
    enfants_parents = {}
    for parent in parents:
        try:
            enfants_parents[str(parent)] = [p.name for p in parent.iterdir()]
        except PermissionError:
            enfants_parents[str(parent)] = ['Permission refusée']

    return {
        "Répertoire actuel": str(current_path),
        "Fichiers": fichiers_actuels,
        "Dossiers": dossiers_actuels,
        "Parents": [str(p) for p in parents],
        "Enfants des parents": enfants_parents
    }

# Interface Streamlit
st.title("Explorateur de Fichiers 🗂️")

data = explorer_repertoire()

st.subheader("📍 Répertoire actuel")
st.code(data["Répertoire actuel"])

st.subheader("📄 Fichiers dans le répertoire actuel")
st.write(data["Fichiers"])

st.subheader("📁 Dossiers dans le répertoire actuel")
st.write(data["Dossiers"])

st.subheader("⬆️ Répertoires parents")
st.write(data["Parents"])

st.subheader("🧾 Enfants des répertoires parents")
for parent, enfants in data["Enfants des parents"].items():
